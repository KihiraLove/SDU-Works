...
from geometry import Problem, PolygonObstacle, Point, Environment, Geometric
...
class Input:
    ...
    def parse_input_file(self) -> Problem:
        ...
        while i < n:
            ...
            if tokens[0].upper() == "OBSTACLE":
                ...
                while i < n:
                    ...
                    self.logger.value_error("EOF reached while reading OBSTACLE: missing END line to close the obstacle.")
        ...
        problem = Problem(Environment(obstacles), start, goal)
        self.verify_start_and_goal(problem)
        return problem
...
class Output:
    ...
    def save_latex(self, latex_document: str) -> None:
        ...
        :param latex_document: string representation of  LaTeX document
        :type latex_document: str
        :return: None
        """
    ...
    def generate_tikz_content(self) -> str:
        ...
        # Draw obstacles
        for polygon in self.result.problem.env.obstacles:
        ...
        # Draw start and goal
        tikz_content += self.result.problem.start.to_tikz(colour="green", p_type="start")
        tikz_content += self.result.problem.goal.to_tikz(colour="magenta", p_type="goal")
    ...
    def generate_pdf(self) -> None:
        ...
        if result.returncode != 0:
            ...
            self.logger.debug(f"pdflatex stderr:\n{result.stderr}")
    ...
    def plot_paths_matplotlib(self) -> None:
        ...
        # Draw obstacles.
        for poly in self.result.problem.env.obstacles:
        ...
        # Draw start and goal.
        ax.plot(self.result.problem.start.x, self.result.problem.start.y, "o", color="green", markersize=8, label="Start")
        ax.plot(self.result.problem.goal.x, self.result.problem.goal.y, "o", color="magenta", markersize=8, label="Goal")
    ...
    def construct_detailed_report(self) -> None:
        ...
        num_obstacles = len(self.result.problem.env.obstacles)
        num_vertices = len(self.result.problem.env.all_vertices())
        ...
        append_to_content("=== Environment summary ===\n"
                          ...
                          f"Start point                         : ({self.result.problem.start.x:.3f}, {self.result.problem.start.y:.3f})\n"
                          f"Goal point                          : ({self.result.problem.goal.x:.3f}, {self.result.problem.goal.y:.3f})\n"
        )
        ...
        append_to_content("=== Grid-based planner (uniform grid) ===\n"
                         ...
                         f"Grid size                            : {self.result.grid_planner.config.grid_size} x {self.result.grid_planner.config.grid_size}\n"
